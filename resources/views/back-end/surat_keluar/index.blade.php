        ...
        <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700">
            ...
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                ...
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        @forelse  ($suratKeluars as $surat_keluar)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                <td class="px-6 py-4">{{ $surat_keluar->nomor_urut }}</td>
                                <td class="px-6 py-4">{{ $surat_keluar->nomor_berkas }}</td>
                                <td class="px-6 py-4">{{ $surat_keluar->alamat_penerima }}</td>
                                <td class="px-6 py-4">{{ $surat_keluar->tanggal }}</td>
                                <td class="px-6 py-4">{{ $surat_keluar->perihal }}</td>
                                <td class="px-6 py-4 text-right">
                                    ...
                                    @if ($role === 'admin')
                                        <a href="{{ route('surat_keluar.edit', $surat_keluar->id) }}"
                                            class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
                                        <form action="{{ route('surat_keluar.destroy', $surat_keluar->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
