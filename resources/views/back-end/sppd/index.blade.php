    ...
    <div class="p-4 sm:ml-64">
        ...
        <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700">
            <div class="flex justify-between items-center mb-4">
                ...
                <form method="GET" action="{{ route('sppd.index') }}" class="flex items-center">
                    ...
                    <tbody>
                        @forelse  ($sppds as $sppd)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                <td class="px-6 py-4">{{ $sppd->nomor_urut }}</td>
                                <td class="px-6 py-4">{{ $sppd->nomor_berkas }}</td>
                                <td class="px-6 py-4">{{ $sppd->tujuans->nama_tujuan }}</td>
                                <td class="px-6 py-4">{{ $sppd->tanggal }}</td>
                                <td class="px-6 py-4">{{ $sppd->keperluan }}</td>
                                <td class="px-6 py-4 text-right">
                                    <a href="{{ route('sppd.show', $sppd->id) }}" class="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-600">Show</a>
                                    @if ($role === 'admin')
                                        <!-- Tambahkan kondisi untuk menampilkan tautan edit dan hapus hanya untuk pengguna biasa -->
                                        <a href="{{ route('sppd.edit', $sppd->id) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
